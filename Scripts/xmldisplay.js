function LoadXML(ParentElementID,XMLElementID) 
{
 	var xmlElm = document.getElementById(XMLElementID);
    xmlDoc = CreateXMLDOM(xmlElm.innerHTML);

	var xmlHolderElement = document.getElementById(ParentElementID);
	if (xmlHolderElement==null) {
		console.log('oops - no element found called ' + ParentElementID);
		return false;
	}
        ...
        for (var i = 0; RootNode.childNodes && i < RootNode.childNodes.length; ++i) {
            if (RootNode.childNodes.item(i).nodeName != '#text') {
                ...
                if ( RootNode.childNodes.length > 1 ) {
	                NodeContent = '';
                } else if ( NodeContent.match(/#!XMLHERE!#/) ) {
	                NodeContent = '';
            		AddTextNode(TagElement, '   ==> Tokenized text goes here') ;
    		        var ContentElement = document.createElement('br');
		            TagElement.appendChild(ContentElement);
                } else {
	                var editableNode = 1;
	            };
				// Now - we also want to find the content of this node, and make it editable
            }                                   
        }                       
...
function ToggleElementVisibility(Element)
{
    ...
    console.log(getXPathForElement(ElementToShow.parentNode, xmlDoc));
    if (ElementToHide) {
