	...
	if ( $act == "export" ) {
		...
		$viewoptions = $tmp['view']; $showoptions = $tmp['show'];
				...
				$maintext .= "
					...
					$countrow
					<div id='mtxt'$tba mod='$action'><text><table $direc>$editxml</table></text></div>
		...
		if ( $username ) {
			$maintext .= "  &bull; <a href='index.php?action=sentedit&cid=$ttxml->fileid&elm=$stype&sid=multi' class='adminpart'>Edit as list</a>";
			$maintext .= "  &bull; <a href='index.php?action=$action&act=export&cid=$ttxml->fileid&elm=$stype' class='adminpart'>Export as JSON</a>";
		};
