<?php
	...
	# Use either sattributes or xattributes
	...
	if ( $settings['cqp']['resnode'] ) {
		$audioelm = $settings['cqp']['resnode'];
	} else if ( is_array($settings['cqp']['xattributes']['sent']) && in_array("audio", array_keys($settings['cqp']['xattributes']['sent'])) ) {
		$audioelm = "s";
	} else if ( is_array($settings['cqp']['xattributes']['text']) && in_array("audio", array_keys($settings['cqp']['xattributes']['text'])) ) {
		$audioelm = "text";
	};
	...
	if ( $act == "freq" ) {
		...
		$context = $_POST['context'] or $context = $_GET['context'] or $context = '6';
		...
		foreach ( $_POST['atts'] as $tmp => $val ) {
			...
			$attitem = $settings['cqp']['xattributes'][$keytype][$keyname]; 
		...
		$maintext .= "<h1 style='text-align: left; margin-bottom: 20px;'>{%Corpus Search}</h1>
			...
			<form action='' method=post id=cqp name=cqp><p>{%CQP Query}: &nbsp; <input name=cql size=80 value='{$cqltxt}'> <input type=submit value=\"Search\"> <a href='index.php?action=$action&act=advanced'>{%advanced}</a>
		...
		if ( $cnt == 0 ) { 
			...
			foreach ( $resarr as $line ) {
				...
				$maintext .= "<tr><td><a href='index.php?action=edit&cid={$fid}'>{$fid}</a><td style='padding-left: 6px; padding-right: 6px; border-left: 1px solid #dddddd;'>".join ( "<td style='padding-left: 6px; padding-right: 6px; border-left: 1px solid #dddddd;'>", $fatts );
			};
			$maintext .= "</table>";
			...
			if ( !file_exists("../bin/tokfind") ) { fatal('tokfind not compiled, please contact admin or use classic style CQP.'); };

			$query = "tabulate Matches $start $end match text_id, match ... matchend id, match[-$context] id, matchend[$context] id";
			$results = $cqp->exec($query);
			$cqltxt = $cql;
			...
			$resarr = split ( "\n", $results ); $scnt = count($resarr);
			$countrow .= "<p>$cnt {%results}";
			if ( $scnt < $cnt ) { 
				$last = min($end,$cnt);
				$countrow .= " &bull; {%Showing} $start - $last";
				if ($end<$cnt) $countrow .= " (<a onclick=\"document.getElementById('rsstart').value ='$end';  document.resubmit.submit();\">{%next}</a>)";
			};
			// $maintext .= "<hr style='color: #cccccc; background-color: #cccccc; margin-top: 6px; margin-bottom: 6px;'>
			//	<table>$matchh";
			...
			foreach ( $resarr as $resline ) {
				...
				if ( $fileid && !file_exists("$xmlfolder/$fileid") ) { 
					...
					if ( $fileid == "" ) {
						$maintext .= "\n<!-- No such XML File: {$oid} -->"; 
					};
				};
		
				if ( in_array ( $_POST['style'], array ( "s", "u", "p" ) ) ) {
					$resnode = $_POST['style']; 
				} else if ( $audioelm && $audioelm != "text" ) {
					$resnode =  $audioelm; 
				} else if ( $_POST['style'] == "kwic" ) {
					$resnode =  "tok"; 
				} else {
					$resnode = $_POST['resnode'] or $resnode = $_GET['resnode'] or $resnode = $settings['cqp']['result']['node'] or $resnode = "tok";
				};
				...
				if ( $fileid ) {
					...
					$m2 = end($idlist); 
					if ( $resnode != "tok" ) { 
						// Do not get more context if we are showing <s> (or anything above <tok>)
						$c1 = $m1; 
						$c2 = $m2; 
					};
			
					$cmd  = "../bin/tokfind --filename='$xmlfolder/$fileid' --begin='$c1' --end='$c2' --expand='$resnode'";
					$resxml = shell_exec($cmd);
					if ( $debug ) {
						$maintext .= "<p>$cmd<p>$m1 - $m2<p>".htmlentities($resxml);
					} else if ( strstr($resxml, '<') ) {
						if ( $resnode == "tok" ) {
							$rescol = "#ffffaa";
						...
						//	$resline = "\n<p><div style='float: left; width: 100px;'><a href='index.php?action=file&cid=$fileid&jump={$idlist[0]}' style='font-size: 11pt;'>$oid</a></div>  $resxml";
							$moreactions .= "\nhllist('$match', 'r-$i', '#ffffff'); ";
						} else {
							// We should not show two identical sentences in a row, just highlight both results in the same one
							if ( $lastres == $resxml && !$settings['cqp']['result']['individual'] ) {
								$ri = $last; $noprint = true; 
								if ( $rescol == "#ffffaa" ) $rescol = "#ffccff";
								else $rescol = "#ffffaa"; 
							} else { 
								$ri = $i; $last = $i; $noprint = false; 
								$rescol = "#ffffaa";
							};
							$resstyle = "style='padding-left: 6px;'";
							$moreactions .= "\nhllist('$match', 'r-$ri', '$rescol'); ";
							$lastres = $resxml;
						};
						if ( !$noprint ) $editxml .= "\n<tr id=\"r-$i\"><td><a href='index.php?action=file&amp;cid=$fileid&amp;jump={$idlist[0]}' style='font-size: 11pt;'>$oid</a></td><td $resstyle>$resxml</td></tr>";
					};
					// $editxml .= "<reslist>".$resline."</reslist>";
				};
		
			};

			if ( !$editxml ) { $editxml = "<p>No results for $cql"; };
			...
			if ( $audioelm && $resnode == $audioelm ) {
				$moreactions .= "\ntoggleaudio();";
			};

			$showform = $_GET['showform'] or $showform = "pform";
			$maintext .= "
		...
		$maintext .= "\n<hr>\n\n<p><form action='index.php?action=download' id=cqlform name=cqlform method=post>
			<input type=hidden name=cql value='$cqlu'></form>";
		...
		if ( !$precql && !$nomatch) { # We actually do want text-based searches
			$maintext .= "<hr>";
 			if ( $minmatchlength == 1 || $fileonly ) {
				...
				$maintext .= "<p>Or run an additional custom CQP command on the results above (Matches):
					...
					<form action='index.php?action=$action&act=freq' id=freqform name=freqform method=post>
						CQP Query:
						<input type=hidden name=cql value='$cqlu'><input name='query' value='group Matches match lemma' size=70>
				...
				$maintext .= "<div style='display: block;' id='freqopts' name='freqopts'>
					...
					<form action='index.php?action=$action&act=freq' id=freqform name=freqform method=post>
						CQP Query:
						<input type=hidden name=cql value='$cqlu'><input name='query' value='group Matches matchend lemma' size=70>
...
// 		if ( $flc > 1 ) $formsel = "<p>{%Form to show}: <select name=showform><option value='word'>{%Written form}</option>$formlist</select> ";
		...
		if ( $cqpatts || $settings['cqp']['xattributes'] ) { $maintext .= "<table cellpadding=5><tr><td valign=top style='border-right: 1px solid #cccccc;'>
			<h3>{%Text Search}</h3>"; };	
		...
		foreach ( $cqpcols as $col ) {
			...
			if ( substr($coldef['type'], -6) == "select" ) {
				...
				$maintext .= "<tr><td$tstyle>{%$colname}
						      ...
						      <td><input name=vals[$col] size=50>";
		};
		
		$maintext .= "</table></div>
				<div name='cqpsearch' id='cqpsearch'>
				<p>{%CQP Query}: &nbsp; <input name=cql size=70 value='{$cql}'> 
				$subheader
				";
		...
		if ( !$audioelm ) {
			...
			if ( $settings['cqp']['xattributes']['sent'] ) {
				$maintext .= "<input type=radio name=style value='sent' onClick=\"switchtype('style', 'sent');\"> {%Sentence}";
			} else if ( $settings['cqp']['result']['node'] ) {
				$restype = $settings['cqp']['result']['node'];
				if ( $restype == "s" ) {
					$maintext .= "<input type=radio name=style value='s' onClick=\"switchtype('style', 'sent');\"> {%Sentence}";
				} else if ( $restype == "u" ) {
					$maintext .= "<input type=radio name=style value='u' onClick=\"switchtype('style', 'sent');\"> {%Utterance}";
				} else if ( $restype == "p" ) {
					$maintext .= "<input type=radio name=style value='p' onClick=\"switchtype('style', 'sent');\"> {%Paragraph}";
				};
			};
		...
		if ( is_array ( $settings['cqp']['xattributes']))
		foreach ( $settings['cqp']['xattributes'] as $xatts ) {
		...
		$maintext .= "<h1 style='text-align: left; margin-bottom: 20px;'>{%Corpus Search}</h1>
			...
			<form action='' method=post id=cqp name=cqp><p>CQP Query: &nbsp; <input name=cql size=80 value='{$cql}'> <input type=submit value=\"Search\"> <a href='index.php?action=$action&act=advanced'>{%advanced}</a></form>
			<script language=Javascript>
			function cqpdo(elm) { document.cqp.cql.value = elm.innerHTML; };
			</script>
			";
	...
	function pattname ( $key ) {
		...
		if ( preg_match ("/^(.*)_(.*?)$/", $key, $matches ) ) {
			...
			if ( $val ) return $val;
			$val = $settings['cqp']['xattributes'][$keytype][$key2]['display'];
			if ( $val ) return $val;
		};
