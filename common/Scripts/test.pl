...
# step 3: open corpus
$C = new CWB::CL::Corpus "TT-POSTSCRIPTUM";
if (defined $C) {
  print " - corpus TT-POSTSCRIPTUM opened\n";
}
else {
  print "!! can't open corpus TT-POSTSCRIPTUM\n";
  exit 1;
}
...
if (not defined $Word) {
  print "!! can't open p-attribute TT-POSTSCRIPTUM.word\n";
  exit 1;
}
print " - default attribute TT-POSTSCRIPTUM.word opened\n";
...
$id = $Word->str2id("almocerve");
if (defined $id) {				# 'almocreve' should not occur in corpus
  print "!! error in str2id() method\n";
  exit 1;
}
print " - word form 'almocerve' not found in corpus\n";

$id = $Word->str2id("almocreve");
if (not (defined $id and $id >= 0)) {		# 'almocreve' should occur, and have freq > 0
  print "!! error in str2id() method\n";
  exit 1;
}
$f = $Word->id2freq($id);
...
print " - word form 'almocreve' (id=$id) occurs $f times\n";
...
$S = $C->attribute("text", 's');			#  's'= s-attribute
$NPh = $C->attribute("text_id", 's');
if ((not defined $S) or (not defined $NPh)) {
  print "!! can't open s-attributes TT-POSTSCRIPTUM.text and POSTSCRIPTUM.text_id\n";
  exit 1;
}
...
print " - opened s-attributes TT-POSTSCRIPTUM.text and POSTSCRIPTUM.text_id [A]\n";

# step 11: number of regions
$N_S = $S->max_struc;
$N_NPh = $NPh->max_struc;
if (not (defined $N_S and defined $N_NPh ) ) {
  print "!! error in max_struc() method\n";
}
print " - $N_S <text> regions and $N_NPh <text_id> regions found\n";
...
$fs2a = CWB::CL::make_set("aaa ��� �a 0", 'split');
$fs2  = "|�a|���|0|aaa|";			# CL ordering uses signed char comparison
